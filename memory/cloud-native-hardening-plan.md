...
1. **Fix backlog.md double-append idempotency** in `research.py` (`append_to_backlog`) and `analytics.py` (`append_feedback_to_backlog`) — both append unconditionally on every run, so a retry/accidental double-run duplicates entries. Agreed design: content-hash-based `append_once(path, key, content)` helper (marker comment `<!-- signal:<hash> -->`), shared between both scripts. This is the *other* idempotency problem, distinct from the gateway's HTTP-retry idempotency keys — needs a *reproducible* key (content hash, or later a caller-supplied run-id), not a random UUID.
2. **Add a unit and mock testing layer** — no test suite exists in this repo at all yet. Scope not yet discussed (framework choice, what to prioritize covering first — likely the gateway/repository/provider-calls modules, since they're newly testable in isolation).
3. **Add pub/sub event hooks** — publish events/event args through a generic subscriber provider layer, console as the default provider, pluggable to external systems like Kafka. A different concern from the observability/logging layer, though following the same provider-plugin pattern (registry + env-var-selected implementation, like `get_provider("ai")`).

None of these three have been started as of this writing.
